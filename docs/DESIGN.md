...
QuorumSeal admits only conventional HTTPS DNS hostnames: direct IP literals, ambiguous numeric IPv4 forms, bracketed IPv6 literals, localhost/local names, userinfo, encoded authorities, and malformed DNS labels are rejected. This is a bounded syntactic admission guard, not DNS resolution or a DNS-rebinding defense; downstream systems should still apply their own network policy.
